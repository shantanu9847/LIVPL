...
#plt.loglog(x,chi2-chi2min,'*')
#plt.show()
#exit(0);

#x = np.linspace(1, 1E5, 10000)
#y = linear(x,params[0],params[1],params[2])
#plt.plot(x, y, '--')
...
theta_guess = [4, 0.1]
x=np.logspace(6,14,20)
params = [optimize.fmin(chi2_n2, theta_guess, args=(E_QG,E, err)) for E_QG in x]
for E_QG in x:
    chi2=[chi2_n2(ps,E_QG,E,err) for  ps in params]
print "x=",x
print "chi2"
print chi2[(chi2<0)]
chi2min=min(chi2) 
print chi2min
plt.loglog(x,chi2-chi2min,'k-*')
...
plt.vlines(x=1.85e+7,ymin=5.0e-05,ymax=4.0,linestyles='dotted',color='magenta',lw=2)
plt.gca().set_ylim(bottom=5.0e-05)
plt.savefig('quadraticLIV.pdf')    
